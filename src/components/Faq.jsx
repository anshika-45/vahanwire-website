...
const FAQItem = ({ question, answer, isOpen, onClick }) => {
    return (
        <div className="bg-white max-w-6xl mx-auto mt-3 sm:mt-4 md:mt-6 rounded-lg mb-2 overflow-hidden border-white ">
            ...
            >
                {question}
                <img loading="lazy" src={isOpen ? minusIcon : plusIcon} alt={isOpen ? "minus" : "plus"} className="w-5 sm:w-6 md:w-7 h-5 sm:h-6 md:h-7 ml-2 flex-shrink-0" />
            </button>
            {isOpen && <p className="px-3 sm:px-4 md:px-5 pb-3 text-gray-500 text-xs sm:text-sm md:text-base">{answer}</p>}
        </div>
    );
};
const FAQ = () => {
    ...
    const faqs = [
        ...
        },
        {
            ...
            answer: "Yes, VahanWire offers 24/7 on-road assistance including towing, jumpstart, flat tire, and emergency fuel delivery — just select the service and share your location."
        },
        {
    ...
    return (
    <div className="px-4 sm:px-8 md:px-16 lg:px-30 bg-[#F4F4F4]">
    <div className="max-w-[1400px] mx-auto px-4 sm:px-6 md:px-8 py-10 sm:py-14 md:py-20">
        <h2 className="text-2xl sm:text-3xl md:text-4xl font-medium mb-4 text-black text-center pb-2 sm:pb-3">Frequently Asked Questions</h2>
    {faqs.map((faq, index) => (
            <FAQItem
                key={index}
