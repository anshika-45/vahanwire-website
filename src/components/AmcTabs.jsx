    ...
    <div className="relative flex flex-col sm:flex-row items-center justify-center sm:justify-between gap-0 sm:gap-0 w-fit sm:w-fit mx-auto mt-4 sm:mt-8 mb-4 sm:mb-8 bg-[#F2F2F2] border-[#d8d8d8] rounded-full">
          ...
          .map((tab) => {
            ...
            return (
              <button
                ...
                className={`relative flex items-center gap-0 px-3 sm:px-3 md:px-6 py-2 rounded-[80px] text-sm sm:text-base md:text-[16px] font-medium transition-all duration-300 h-10 sm:h-12 md:h-[50px] whitespace-nowrap
                  ${isSelected ? "text-white bg-[#3B77D5]" : showRemoveFilter ? "text-[#344054] cursor-not-allowed" : "text-[#344054]"}
                `}
              >
                <img
                  src={vehicleType === "bike" ? (isSelected ? bikeIcon : bikeIconDark) : (isSelected ? carIcon : carIconDark)}
      ...
      {showRemoveFilter && (
        <button
          ...
          className="flex items-center gap-0 px-2 sm:px-3 md:px-6 py-2 rounded-[80px] text-[#172E53] bg-[#F2F2F2] font-medium transition-all duration-300 h-10 sm:h-12 md:h-[50px] text-sm sm:text-base md:text-[16px] whitespace-nowrap"
        >
          X Remove Filter
        </button>
      )}
