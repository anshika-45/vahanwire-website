...
const PlanSummaryPage = ({ isOpen, onClose, onBack }) => {
  ...
  // Disable body scroll when success popup is open
  useEffect(() => {
  ...
  const planDetails = {
    title: "Car Luxury Plan",
    price: "₹999",
    note: "*Your RSA Plan starts on 07 October 2025, 10:06 pm and stays active for 365 days.",
  };
    ...
    if (currentView === "payment") {
      return (
        <Payment
          ...
          onPaymentSuccess={() => setCurrentView("success")}
        />
      );
    }

    if (currentView === "success") {
      return <SuccessPurchase onClose={onClose} />;
    }
      ...
      <div className="flex flex-col gap-6">
        ...
        <div className="bg-white rounded-xl px-6 py-5 border border-gray-200">
          ...
          type="text"
            ...
            />
            <Button text="Apply" className="bg-blue-600 text-white px-5 py-2 rounded-lg font-semibold hover:bg-blue-700" />
          </div>
        </div>
  ...
  if (currentView === "success") {
    return (
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
        <SuccessPurchase onClose={onClose} />
      </div>
    );
  }
  ...
  return (
    <Modal
      ...
      onClose={onClose}
      onBack={
        currentView === "payment" ? () => setCurrentView("summary") : onBack
      }
      proceedButton={
        currentView === "summary" ? (
          <Button
